...
% Make global struct of integration kernels and vectors.
...
for iter=restart:loops
    ...
    %Goto Optimum Diffusion point every 5 iterations.
    if( ~mod(iter,5) )
        options4 = optimset('Display','iter-detailed','GradObj','on','TolFun',1e-4,...
    ...
    % Get the expected Peq (E-step) and the diffusion derivative.
    [pyB,gradDB,dPhi,Phi,AB]=getPhi(Fdata,pB.^2,ldB,Chebs,pyNorm);

    % set the optimal peq to be equal to the observed peq (M-step)
    ...
    [AX,H1,H2] = plotyy(xref,[pB.^2 pCP.^2],xref,[dPhi]);
